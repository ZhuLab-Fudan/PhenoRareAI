import sys
import txt2hpo_rank_bert_base_medical_medical_ner as txt2hpo
import pickle
...
datacohort='_bert_base_medical_medical_ner'
# raredisease="LOPD"
...
path_bert_base_result="bert_base_chinese_medical_result/medical_ner_result"


patient_already_list=[]
filessss = os.listdir(path_bert_base_result)
...
for file in files:
    ...
    if patient_name not in patient_already_list:
        ...
        path_allgram = "."
        data_allgram = pd.read_csv(path_allgram + "/" + "bert_base_chinese_medical_lopdsma_elements.csv")
        # patient_fill,allngram_text,index,index_place,label,score
        for i in range(0, data_allgram.shape[0]):

            patient_allgramtext = data_allgram["word"][i]
            patient_idmap = data_allgram["patient"][i]
            patient_beginid = data_allgram["beginid"][i]
            patient_endid = data_allgram["endid"][i]
        ...
        for i in range(0, len(given_hpos)):
            ...
            score = score_chinese
            # if score > 0:  # and hpo in filter_list:
        ...
        fo_csv.to_csv(path_bert_base_result + "/" + patient_name + "-rank.csv", index=None)
