import sys
import txt2hpo_rank_cutoff_medical_ner as txt2hpo
...
datacohort='_cutoff_medical_ner'
# raredisease="LOPD"



with open("bert-base-chinese-medical-ner_feature_chpo_phenopro_core_dict.json") as fp:
    chinese_feature_chpo= json.load(fp)



tokenizer_chinese = AutoTokenizer.from_pretrained("bert-base-chinese-medical-ner")

model_chinese = AutoModel.from_pretrained("bert-base-chinese-medical-ner")
...
path = "cutoff_result/medical_ner"
os.makedirs(path)

path_csv_result="cutoff_result/medical_ner"

patient_already_list=[]
filessss = os.listdir(path_csv_result)
...
for file in files:
    ...
    if patient_name not in patient_already_list:
        input_dir = path_data + "/" + patient_name + ".txt"

        # output_dir = "/public/home/zhaiwq/tmp/HPONEREL_pycharm_project/PhenoPro-master/TXT2HPO/allngram_result/LOPD/" + patient_name + "-rank.txt"

        elements_all = txt2hpo.splitting(input_dir, HPOs, chpo_dic_dir, split_punc_dir, rm_dir)
        #
        #
        elements = []
        for i in elements_all:
            if i not in StopWordtmp:
                elements.append(i)
        #
        ...
        for i in range(0, len(given_hpos)):
            ...
            beginid = doucument_ner.find(element, tqdmlenth)
            if beginid >= 0:
                endid = beginid + len(element)
                tqdmlenth = beginid
                ...
                score_chinese = 1 - spatial.distance.cosine(chinese_feature_vectors_1,
                                                               chinese_feature_vectors_2)
                ...
                beginplace_list.append(beginid)
                endplace_list.append(endid)
        ...
        # fo.write('#Given_HPO\tHPO_name\tHPO_name_cn\tElement\tscore\n')
        fo_csv.to_csv(path_csv_result + "/" + patient_name + "-rank.csv", index=None)
