...
path = "hospital_result"
os.makedirs(path)

outpath = "hospital_result"
#读取path路径下的全部文件名
...
for file in files:
    ...
    Disease_ranking_result.to_csv('hospital_result/%s'%(name_str),index=False,header=True)


path_1 = "hospital_result"
...
for file in filess:
    ...
    with open("hospital_result/%s"%(name_str)) as fp:
        for line in fp:
...
with open("Phrank_hospital_result.json", 'w') as fp:
